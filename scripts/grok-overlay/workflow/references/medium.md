...
- **primary-tree read-only:** use one `explore` (or read-only `plan`/`general-purpose`)
  analysis stream; do not spawn an implementer.
- **disposable-worktree-only:** use one `executor` only with
  `isolation="worktree"` (or an equivalent verified disposable worktree); never
  edit or build the primary tree.
- **authorized primary-tree mutation:** use one `executor` (`general-purpose`,
  write-capable) that may edit and run checks only within its assigned ownership
  surface and effective authorization.

1. Spawn `plan` (fallback `general-purpose`, `capability_mode=read-only`) as
   `workflow_controller` with the controller contract and obtain a bounded plan.
...
4. Send its evidence to the same controller for review (`resume_from` the
   controller id when continuing the same control thread).
5. Route production defects back to the same implementer. Add a `verifier`
   (`general-purpose`, read-only or execute) only when behavior or risk warrants
